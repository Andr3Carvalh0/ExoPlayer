...
public final class ParsableByteArray {
   ...
   */
  public int readInt() {
    return (data[Math.min(position++, data.length - 1)] & 0xFF) << 24
        | (data[Math.min(position++, data.length - 1)] & 0xFF) << 16
        | (data[Math.min(position++, data.length - 1)] & 0xFF) << 8
        | (data[Math.min(position++, data.length - 1)] & 0xFF);
  }
